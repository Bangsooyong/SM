...
<body>
	...
	<h1>게시글 상세 보기</h1>
	<form action="update">
		글번호<br /> <input type="text" id="bno" name="bno"
			value="${boardVO.bno }" readonly /><br /> 제목<br /> <input
			type="text" value="${boardVO.title }" readonly /><br /> 본문<br />
		<textarea rows="5" cols="40" readonly>${boardVO.content }</textarea>
		<br /> 작성자 아이디<br /> <input type="text" value="${boardVO.userid }"
		...
		<input type="text" value="${regdate }" readonly /><br /> <input
			type="submit" value="수정" /> <input type="button" id="btnList"
			value="메인으로 돌아가기" />
						...
						function() {
							...
							function getAllReplies() {
								...
								$
										.getJSON(
												url,
												function(result) {//getJSON URL주소로 get방식으로 요청을하고 성공시 전달받은 값을 매개변수로받음
													console.log("댓글 개수: "
															+ result.length);
													var list = "";
													$(result)
