...
<body>
	...
	<div>
		<h3>QnA게시판</h3>
		<button id="btnLogin">판매자 로그인</button>
		<table>
			<tr>
				<th>문의유형&emsp;</th>
				<th>답변여부&emsp;</th>
			...
			<c:forEach var="list" items="${listQnA }" varStatus="state">
				<tr>
					...
					<textarea cols="25" rows="3" class ="qnaDetail" modData="${state.index }" readonly>
					${list.qna_cont }&emsp;
					</textarea>
					<br/>
					...
					<c:if test="${list.qna_reply eq 0 }">
						...
						&emsp;&emsp;&emsp;<b>판매자 답변</b><br/>
							...
							<button type="submit" id="insertReply">저장${list.qna_no }</button>
						</div>
					...
					<c:if test="${list.qna_no eq listr.qna_no }">
						...
						&emsp;&emsp;&emsp;<b>판매자 답변${state.index }</b><br/>
						...
						<button type="submit" class="updateReply1" disabled>수정완료</button>
						<button type="submit" class="deleteReply">삭제/${listr.qna_r_no }/${listr.qna_no }</button>
						</div>
						</c:if>
						</c:forEach>
					</c:if>
...
/* 				var num = $('.replies').attr('repData');
					$('div[modData = '+num+']').show(); */
				if(x == 0) {
				$('.modify').show();
				x = 1;
				$('#btnLogin').text('판매자 로그아웃');
				} else {
				$('.modify').hide();
				x = 0;
				$('#btnLogin').text('판매자 로그인');
				}
			});
			...
			$('.updateReply').click(function () {
				...
				$('.updateReply1').attr("disabled", false);
			});
